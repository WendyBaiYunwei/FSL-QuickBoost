...
def mini_imagenet_folders():
    config = Config()
    train_folder = config.data_dir + 'train'
    test_folder = config.data_dir + 'test'
...
def get_mini_imagenet_data_loader_resnet(task, num_per_class=1, split='train',shuffle = False):
    normalize = transforms.Normalize(mean=[0.92206, 0.92206, 0.92206], std=[0.08426, 0.08426, 0.08426])

    dataset = MiniImagenet(task,split=split,transform=transforms.Compose([transforms.Resize(80),transforms.ToTensor(),normalize]))
